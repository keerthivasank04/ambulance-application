...
// Service area: All districts across Tamil Nadu
...
router.post('/', (req, res) => {
  ...
  if (lat < TN_BOUNDS.minLat || lat > TN_BOUNDS.maxLat || lng < TN_BOUNDS.minLng || lng > TN_BOUNDS.maxLng) {
    return res.status(400).json({ error: 'Location is outside the Tamil Nadu emergency coverage area. Please call 108 directly.' });
  }
